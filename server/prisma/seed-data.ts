...
const ticketSeeds = [
  ...
  {
    ticketNumber: 'TKT-20260924-00000009',
    submissionKey: '30000000-0000-4000-8000-000000000009',
    requesterKey: requesterSeeds[0].fixtureKey,
    ownerKey: staffSeeds[0].fixtureKey,
    category: 'Software',
    relatedSystem: 'LEB2 App',
    summary: 'Lab 4 recently resolved dashboard fixture',
    requestedPriority: RequestedPriority.MEDIUM,
    status: TicketStatus.RESOLVED,
    workCycle: 1,
    resolvedAt: new Date('2026-09-23T04:00:00.000Z'),
    description: 'Reserved fixture for the recently resolved dashboard calculation.',
  },
  {
    ticketNumber: 'TKT-20260924-00000010',
    submissionKey: '30000000-0000-4000-8000-000000000010',
    requesterKey: requesterSeeds[1].fixtureKey,
    ownerKey: staffSeeds[1].fixtureKey,
    category: 'Network',
    relatedSystem: 'VPN',
    summary: 'Lab 4 waiting for requester fixture',
    requestedPriority: RequestedPriority.HIGH,
    status: TicketStatus.WAITING_FOR_REQUESTER,
    workCycle: 1,
    resolvedAt: null,
    description: 'Reserved fixture that requires additional requester information.',
  },
  {
    ticketNumber: 'TKT-20260924-00000011',
    submissionKey: '30000000-0000-4000-8000-000000000011',
    requesterKey: requesterSeeds[2].fixtureKey,
    ownerKey: null,
    category: 'Hardware',
    relatedSystem: 'Corporate Laptop',
    summary: 'Lab 4 unassigned urgent fixture',
    requestedPriority: RequestedPriority.URGENT,
    status: TicketStatus.NEW,
    workCycle: 1,
    resolvedAt: null,
    description: 'Reserved fixture for unassigned and urgent dashboard calculations.',
  },
  {
    ticketNumber: 'TKT-20260924-00000012',
    submissionKey: '30000000-0000-4000-8000-000000000012',
    requesterKey: requesterSeeds[3].fixtureKey,
    ownerKey: staffSeeds[2].fixtureKey,
    category: 'Account and Access',
    relatedSystem: 'Email',
    summary: 'Lab 4 reopened work-cycle fixture',
    requestedPriority: RequestedPriority.HIGH,
    status: TicketStatus.REOPENED,
    workCycle: 2,
    resolvedAt: null,
    description: 'Reserved fixture with historical work from a prior work cycle.',
  },
] as const

export const actionSeeds = [
  {
    fixtureKey: 'lab4-action-single-completed',
    ticketNumber: 'TKT-20260911-00000002',
    ticketWorkCycle: 1,
    status: ActionStatus.COMPLETED,
    description: 'Reviewed wireless controller logs and reset the affected access point.',
    result: 'The connection remained stable after the access point reset.',
    creatorKey: staffSeeds[0].fixtureKey,
    performerKey: staffSeeds[0].fixtureKey,
    assigneeKey: staffSeeds[0].fixtureKey,
    followUpRequired: false,
    followUpNote: null,
    attachmentNotes: 'Controller log retained in the local lab evidence set.',
    ...
    actionAt: new Date('2026-09-22T03:00:00.000Z'),
    createdAt: new Date('2026-09-22T02:50:00.000Z'),
    completedAt: new Date('2026-09-22T03:05:00.000Z'),
    cancelledAt: null,
  },
  {
    fixtureKey: 'lab4-action-many-planned',
    ticketNumber: 'TKT-20260911-00000003',
    ticketWorkCycle: 1,
    status: ActionStatus.PLANNED,
    description: 'Schedule a battery health diagnostic after the current class.',
    result: null,
    creatorKey: staffSeeds[0].fixtureKey,
    performerKey: null,
    assigneeKey: staffSeeds[1].fixtureKey,
    followUpRequired: true,
    followUpNote: 'Confirm a suitable diagnostic window with the requester.',
  ...
  {
    fixtureKey: 'lab4-action-many-progress',
    ticketNumber: 'TKT-20260911-00000003',
    ticketWorkCycle: 1,
    status: ActionStatus.IN_PROGRESS,
    description: 'Run the manufacturer battery diagnostic and collect the report.',
    result: null,
    creatorKey: staffSeeds[1].fixtureKey,
    performerKey: null,
    assigneeKey: staffSeeds[2].fixtureKey,
    followUpRequired: false,
    followUpNote: null,
    attachmentNotes: 'Diagnostic report will be referenced after completion.',
    ...
    actionAt: null,
    createdAt: new Date('2026-09-22T04:10:00.000Z'),
    completedAt: null,
    cancelledAt: null,
  },
  {
    fixtureKey: 'lab4-action-old-cycle-completed',
    ticketNumber: 'TKT-20260924-00000012',
    ticketWorkCycle: 1,
    status: ActionStatus.COMPLETED,
    description: 'Reset the mailbox session before the issue later recurred.',
    result: 'The mailbox opened successfully during the first work cycle.',
    creatorKey: staffSeeds[2].fixtureKey,
    performerKey: staffSeeds[2].fixtureKey,
    assigneeKey: staffSeeds[2].fixtureKey,
    followUpRequired: false,
    followUpNote: null,
    attachmentNotes: null,
    ...
    actionAt: new Date('2026-09-21T05:00:00.000Z'),
    createdAt: new Date('2026-09-21T04:50:00.000Z'),
    completedAt: new Date('2026-09-21T05:05:00.000Z'),
    cancelledAt: null,
  },
] as const
...
export async function seedDatabase(prisma: PrismaClient) {
  ...
  for (const seed of ticketSeeds) {
    ...
    const row = await prisma.ticket.upsert({
      ...
      create: {
        ...
        status: seed.status,
        ...('workCycle' in seed
          ? { workCycle: seed.workCycle, resolvedAt: seed.resolvedAt }
          : {}),
      },
    })
  ...
  for (const seed of actionSeeds) {
    const ticketId = tickets.get(seed.ticketNumber)
    const creator = users.get(seed.creatorKey)
    const performer = seed.performerKey ? users.get(seed.performerKey) : null
    const assignee = seed.assigneeKey ? users.get(seed.assigneeKey) : null
    if (!ticketId || !creator || (seed.performerKey && !performer) || (seed.assigneeKey && !assignee)) {
      throw new Error('Lab 4 Action seed references are incomplete.')
    }

    await prisma.actionTaken.upsert({
      where: { fixtureKey: seed.fixtureKey },
      update: {},
      create: {
        fixtureKey: seed.fixtureKey,
        ticketId,
        ticketWorkCycle: seed.ticketWorkCycle,
        status: seed.status,
        actionAt: seed.actionAt,
        description: seed.description,
        result: seed.result,
        ...
        attachmentNotes: seed.attachmentNotes,
